...
1) Open up your "data_analysis_tutorial" R Project that you created in the last lesson - if you didn't do this, [go back and do it now](L10-data-analysis-1-data-frames.html#41_r_setup).
2) Create a new `.R` file (File > New File > R Script), and save it as "`data_wrangling.R`" inside your "data_analysis_tutorial" R Project folder.
...
download.file(
    ...
    destfile = here('data', 'wildlife_impacts.csv')
)
```
...
library(dplyr)
library(here)
df <- read_csv(here('data', 'wildlife_impacts.csv'))
```
...
<img src="images/horst_monsters_data_wrangling.png" width=500> Art by [Allison Horst](https://github.com/allisonhorst/stats-illustrations)
...
```{r, eval=FALSE}
write_csv(heightSummary, path = here('data_output', 'heightSummary.csv')
```
